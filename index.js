            ...
            .then(({ id }) => {
                msg.channel.send(checkUserEmbed).then(msge => {
                    if (id != undefined) {
                        ...
                        fetch("https://script.tftech.de/api/hggv2/?key=" + config.hgg_hey + "&uuid=" + id)
                            .then(result => result.json())
                            .then(({ reports }) => {
...
function botinfo_cmd(msg, args) {
    ...
    embed4.addField('Developer', '<@702790608649060432>')
    embed4.addField('Hilfe-Befehl', 'r!help');
    embed4.addField('Letztes Update', '16.04.2021 | 22:42');
    embed4.addField('Version', '0.0.4');
